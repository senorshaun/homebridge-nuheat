...
async makeAPICall(callURL, callOptions = {}) {
        ...
        if(!response) {
            if (this.debug) {
                this.log("NuHeatAPI: Unable to make API call. Acquiring a new access token.");
            }
            this.accessToken = null;
            return false;
        }
    ...
    async getOAuthToken() {
        ...
        // Call the NuHeat authorization endpoint u to get the web login page.
        ...
        if(response.headers.get("location").startsWith('/connect/authorize/callback?')) {
    ...
    async refreshOAuthToken() {
        ...
        if (this.debug) {
            this.log("NuHeatAPI: Successfully refreshed the NuHeat API access token.");
        }
    ...
    async acquireAccessToken(){
        ...
        if(firstConnection) {
            ...
            if (this.debug) {
                this.log("NuHeatAPI: Successfully reacquired a NuHeat API access token.");
            }
        }
    ...
    async refreshAccessToken() {
        ...
        if(!this.accessToken) {
                if (this.debug) {
                        this.log('NuHeatAPI: Acquiring new access token. Ours seems to be missing');
                }
        ...
        if((Date.now() - this.accessTokenTimestamp) < (this.refreshInterval * 1000)) {
            ...
            if (this.debug) {
                this.log('NuHeatAPI: Acquiring new access token. Ours has expired or is expiring soon');
            }
            ...
            if(error instanceof FetchError) {
                switch(error.code) {
                    ...
                    case "ECONNRESET":
                        ...
                        if(!isRetry) {
                            if (this.debug) {
                                this.log("NuHeatAPI: Connection has been reset. Retrying the API action.");
                            }
                            return this.fetch(url, options, decodeResponse, true);
                        }
